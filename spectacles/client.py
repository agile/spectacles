...
class LookerClient:
    ...
    def git_branch(self, project: str, branch: str, ref: str = None) -> None:
        """Checks out and/or reset --hard an existing branch.
           ...
           Note that specifying a ref (sha) will result in any uncomitted
        ...
        if branch == "master":
            logger.debug(f"git branch operations not applicable to master branch.")
            return
    ...
    def reset_to_remote(self, project: str, branch: str) -> None:
        """Reset state of Git branch in Looker to match remote.
           ...
           Note that any uncomitted changes to lookml files made within looker
